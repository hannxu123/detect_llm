# functions used to add watermark in a black-box setting for detection, for more information, see https://arxiv.org/pdf/2305.08883.pdf
import numpy as np
import os
...
class Black_box_watermark:
    def __init__(self, cache_dir, language='English', mode='fast', tau_word=0.8, lamda=0.83, n_samples=50):
    ...
    def main_func(self, data, model_collector, results_path):
        ...
        self.model = model_collector.base_model.to(self.device)
        self.tokenizer = model_collector.base_tokenizer
        self.relatedness_model = model_collector.mask_model.to(self.device)
        self.relatedness_tokenizer = model_collector.mask_tokenizer

        sampled_text = data["sampled"][:self.n_samples]

        if self.mode == 'fast':
            detect_method = self.watermark_detector_fast
        else:
            detect_method = self.watermark_detector_precise            

        real_preds = []
        fake_preds = []
        for text in sampled_text:
            is_watermark, p_value, n, ones, z_value = detect_method(text)
            real_preds.append(p_value)
            watermarked_text = self.embed(text)
            is_watermark, p_value, n, ones, z_value = detect_method(watermarked_text)
            fake_preds.append((1 - p_value))

        output = run_detect_experiment(real_preds, fake_preds, self.n_samples)
...
def run_detect_experiment(real_preds, fake_preds, n_samples):
    predictions = {
        'real': real_preds,
        'samples': fake_preds,
    }
