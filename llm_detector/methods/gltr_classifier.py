...
def construct_dataset(original_train, sampled_train, data_path, cache_dir, shuffle=True):
    ...
    dataset = load_dataset('json', data_files=data_path, split='train', cache_dir=cache_dir)
...
class GLTR_classifier:
    ...
    def train_and_test_func(self, data):
        ...
        data_path = os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_train_features.json')
        if os.path.exists(data_path):
            used_dataset = load_dataset('json', data_files=data_path, split='train', cache_dir=self.cache_dir)
        else:
            original_train, sampled_train, _, _ = load_dataset_for_gltr_classifier(data)
            init_dataset = construct_dataset(original_train, sampled_train, os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_init_train_data.json'), self.cache_dir, True)
        ...
        train_lr_classifier(x, y, os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_best_model.pkl'))

        # load best checkpoint
        with open(os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_best_model.pkl'), 'rb') as file:
            self.classification_model = pickle.load(file)

        # test GLTR classifier
        data_path = os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_test_features.json')
        if os.path.exists(data_path):
            used_dataset = load_dataset('json', data_files=data_path, split='train', cache_dir=self.cache_dir)
        else:
            _, _, original_test, sampled_test = load_dataset_for_gltr_classifier(data)
            init_dataset = construct_dataset(original_test, sampled_test, os.path.join(save_path, self.dataset_name + '_' + self.base_model + '_init_test_data.json'), self.cache_dir, False)
            used_dataset = predict_data(init_dataset, data_path, self.base_model, self.base_tokenizer, self.device, params.OTHERS_PARAMS['test_batch_size'], params.OTHERS_PARAMS['padding_max_length'])
