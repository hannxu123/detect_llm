...
from utils import load_texts, strip_newlines, process_spaces
import params
import importlib
...
def load_user_dataset(data_folder):
    try:
        original_text = load_texts(os.path.join(data_folder, 'human_written_data.jsonl'))
        sampled_text = load_texts(os.path.join(data_folder, 'model_generated_data.jsonl'))
    except:
        raise ValueError(f'Cannot load data from the folder {data_folder}. Please save data files "human_written_data.jsonl" and "model_generated_data.jsonl" into the folder {data_folder} first.')
...
def post_clean_dataset(original_text, sampled_text):
    ...
    original_text = [strip_newlines(x) for x in original_text]
    sampled_text = [strip_newlines(x) for x in sampled_text]

    original_text = [process_spaces(x) for x in original_text]
    sampled_text = [process_spaces(x) for x in sampled_text]
