...
df1 = pd.read_csv('./gpt_data/imdb/title.akas.tsv.gz', compression='gzip', delimiter='\t')
...
df2 = pd.read_csv('./gpt_data/imdb/title.ratings.tsv.gz', compression='gzip', delimiter='\t')
...
for i in range(20):
    print(titles[i])
    sense = random.choice(['enjoyable', 'just OK', 'mediocre', 'unpleasant'])
    prompt = random.choice(['It is because that', 'The reason is that', 'I just feel ', 'I am feeling that '])
